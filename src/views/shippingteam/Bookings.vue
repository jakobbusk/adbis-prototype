<template>
  <ShippingLayout>
      ...
      <div >
          <div v-show="tabs[0].current || tabs[1].current || tabs[2].current || tabs[3].current" >
            <table class="w-full text-left">
                ...
                <tr>
                    ...
                    <th v-show="tabs[2].current">Assign resposible</th>
                ...
                <tr v-for="booking in filteredBookings" :key="booking.id" class="border-b theme-soft-border">
                    ...
                    <td v-show="booking.shippingResponsible == '' && tabs[2].current">
                        <select class="block w-full rounded-md theme-surface px-2 py-0.5 text-base theme-text outline outline-1 -outline-offset-1 theme-soft-border focus:outline-2 focus:-outline-offset-2 focus:theme-border sm:text-sm/6">
                            <option value="">Unassigned</option>
                            <option value="John Doe">John Doe</option>
                            <option value="Jane Doe">Jane Doe</option>
                            <option value="Sarah Chen">Sarah Chen</option>
                            <option value="Martin Lund">Martin Lund</option>
                            <option value="Nadia Olsen">Nadia Olsen</option>
                            </select>

                    </td>
                    <td class="py-1.5 text-right">
                        <router-link :to="'/shipping/bookings/' + booking.bookingID" class="text-sm/6 font-semibold theme-text hover:opacity-90">View<span class="sr-only">, {{ booking.bookingID }}</span></router-link>
                    </td>
                </tr>
                </tbody>
            </table>
...
// function to calculate counts for each tab based on bookings data
function calculateTabCounts() {
  const myBookingsCount = bookings.filter(booking => booking.shippingResponsible === 'John Doe').length;
  const allBookingsCount = bookings.length;
  const unassignedCount = bookings.filter(booking => !booking.shippingResponsible).length;
  const completedCount = bookings.filter(booking => booking.status.code === 'completed').length;

  tabs.value[0].count = myBookingsCount;
  tabs.value[1].count = allBookingsCount;
  tabs.value[2].count = unassignedCount;
  tabs.value[3].count = completedCount;
}
...
// Only show "John Doe" bookings for "My Bookings" tab, show all bookings for "All Bookings" tab, and only unassigned bookings for "Unassigned" tab. "Completed" tab can be implemented similarly by filtering bookings with status 'completed'.
function filterBookings() {
  if (tabs.value[0].current) {
    return bookings.filter(booking => booking.shippingResponsible === 'John Doe');
  } else if (tabs.value[1].current) {
    return bookings; // Show all bookings
  } else if (tabs.value[2].current) {
    return bookings.filter(booking => !booking.shippingResponsible); // Show unassigned bookings
  } else if (tabs.value[3].current) {
    return bookings.filter(booking => booking.status.code === 'completed'); // Show completed bookings
  }
  return [];
}
...
calculateTabCounts(); // Initial count calculation
