      ...
      <div class="flex h-16 items-center justify-between">
        ...
        <div class="hidden md:block">
          <div class="flex items-center gap-x-4 ">
            <router-link :to="{ name: 'ShippingNewBooking' }" >
...
async function fetchTimeline() {
  try {
    ...
    // Map API data to activityItems format
    activityItems.value = data.map(entry => ({
      id: entry.id,
      name: entry.message,
      comment: `By ${entry.sender} (${entry.type})`,
      date: new Date(entry.createdAt).toLocaleString(),
      viewed: false, // You can implement logic to determine if it's viewed
    }))
  } catch (error) {
